...
#define SLAVE_ADDR 0x23 // set salve addres 0x23
volatile byte myData[4]; // create 4 varaibles for input values
volatile bool flag = false; // create a variable for if there is any incoming data
//create 4 servo objekts
Servo snurr;
Servo arm;
Servo vinkel;
Servo klo;
//create a bunch of varaiables for servo posistions
float snurrout;
int snurrmellan;
float armout;
int armmellan;
...
float kloout;
int klomellan;
// create som varaibles for wait without dealy() loop
...
void setup()
{
  ...
  Wire.begin(SLAVE_ADDR); // start i2c as slave with slave adress
  Wire.onReceive(receiveEvent); 
  //attach servos and set position to 0
  snurr.attach(11);
  arm.attach(10);
  vinkel.attach(9);
  klo.attach(6);
  ...
  vinkel.write(0);
  klo.write(0);
  pinMode(13, OUTPUT); //set pin of led to output
}

void loop()
{
  //loop to set time true every 1500 secounds (doesent work )
